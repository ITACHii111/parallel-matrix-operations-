...
__global__ void per_row_AB_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    ...
    for(long int i = 0; i < n; i++){
        for(long int j = 0; j < n; j++){
            long int id = A[blockIdx.x * n + i] * B[threadIdx.x * n + j]; 
            C[blockIdx.x * n * m * n + i * m + j * n * m + threadIdx.x] = id;
        }
    }
...
__global__ void per_column_AB_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    // --> Complete the kernel ....
    long int id = threadIdx.x * blockDim.y + threadIdx.y;
    if(id < n){
        
        for(long int i = 0; i < m; i++){
            for(long int j = 0; j < m; j++){
                long int temp = A[i * n + blockIdx.x] * B[id + j * n];
                C[i * n * m * n + blockIdx.x * m + id * m * n + j] = temp;
            }
        }
    }
...
__global__ void per_element_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    ...
    if(id < m * n * m * n){
        ...
        C[id] = A[(temp1 / n) * n + (temp2 / m)] * B[(temp2 % m) * n + (temp1 % n)];
    }
}
...
int main(int argc,char **argv){
    ...
    // --> Launch the kernel 
    per_row_AB_kernel<<<gridDimx, gridDimy>>>(d_a, d_b, d_c, m, n);
    ...
    // --> Set the launch configuration 
    dim3 grid1(n, 1, 1);
    dim3 block1(5, ceil((float)n/5), 1);


    starttime = rtclock(); 

    // --> Launch the kernel 
    per_column_AB_kernel<<<grid1, block1>>>(d_a, d_b, d_c, m, n) ;
    cudaDeviceSynchronize(); 
