...
__global__ void computeMatrix(int *A, int *B, int *E, int p, int q, int r){
  ...
  __syncthreads();
	...
	for(int k = 0; k < q; k++){
		...
		int temp = A[blockIdx.x * q + k] * B[k * r + threadIdx.x];
		sE[threadIdx.x] += temp;
		__syncthreads();
	}

	E[blockIdx.x * r + threadIdx.x] = sE[threadIdx.x];

	
}

__global__ void computeTransposeMatrix(int *C, int *D, int *E, int p, int q, int r){
	...
	sE[threadIdx.x] = C[blockIdx.x * q + threadIdx.x] * D[blockIdx.y * q + threadIdx.x];
	__syncthreads();

	if(threadIdx.x == 0){
		for(int i = 0; i < q; i++){
			E[blockIdx.x * r + blockIdx.y] += sE[i];
		}
	}
}
...
void computE(int p, int q, int r, int *h_matrixA, int *h_matrixB, 
	...
	computeMatrix<<<p, r>>>(d_matrixA, d_matrixB, d_matrixE, p, q, r);
	cudaDeviceSynchronize();


	computeTransposeMatrix<<<grid,q>>>(d_matrixC, d_matrixD, d_matrixE, p, q, r);
	cudaDeviceSynchronize();
